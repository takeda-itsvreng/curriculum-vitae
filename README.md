...
- 電子債権システム更改
  ...
  - 特記事項：
    - 詳細設計
①検証環境構築：Oracle VM VirtualBoxを用いて、右記をインストール
②詳細設計書ベンダレビュー：レビューア、指摘取り込み確認の担当者割り振りなど
③現行システム課題改善対応：Cygwin未使用化検討、パッチ適用方針検討、ログメンテナンス運用検討など
...
- インターネットバンキングシステム保守・運用
  ...
  - 特記事項：
    - 詳細設計
①機能追加：JP1ジョブ作成、OVOテンプレート追加、スタティックルーティング追加、LVOL作成など
②保守作業：OSパッチ適用、リソース評価資料作成、ピーク日リソース状況監視など
③障害対応：電源モジュール交換、ディスク交換など
④上記作業にともなう各種調整：各業務グループとの作業調整、ハードウェアベンダへの調査依頼・交換作業調整など
⑤システム更改初期検討：Oracleインスタンス構成検討、開発環境構成検討
